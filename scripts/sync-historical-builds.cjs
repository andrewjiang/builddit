...
const BuildClaim = mongoose.models.BuildClaim || mongoose.model('BuildClaim', BuildClaimSchema);
const FarcasterUser = mongoose.models.FarcasterUser || mongoose.model('FarcasterUser', FarcasterUserSchema);
...
async function syncCasts(cursor, limit = 50) {
    ...
    let totalTaggedClaims = 0;
                ...
                embeds: cast.embeds.map(embed => ({
                    ...
                    cast: embed.cast ? {
                        author: embed.cast.author ? {
                            ...
                            displayName: embed.cast.author.display_name || embed.cast.author.username || '',
            ...
            if (replies.length > 0) {
                const taggedReplies = replies.filter(reply => isTaggedBuild(reply.text));
                console.log('\nProcessing Replies:', {
                    total: replies.length,
                    tagged: taggedReplies.length,
                    engagement: replies.reduce((acc, reply) => ({
            ...
            if (recasts.length > 0) {
                const taggedQuotes = recasts.filter(recast => recast.cast && isTaggedBuild(recast.cast.text));
                console.log('\nProcessing Recasts:', {
                    total: recasts.length,
                    tagged: taggedQuotes.length,
                    engagement: recasts.reduce((acc, recast) => ({
                        likes: acc.likes + (recast.cast?.reactions.likes_count || 0),
                        recasts: acc.recasts + (recast.cast?.reactions.recasts_count || 0),
                    }), { likes: 0, recasts: 0 }),
                });
            }
                ...
                await BuildClaim.findOneAndUpdate(
                    { castHash: recast.cast.hash },
                    {
                        ...
                        author: {
                            ...
                            displayName: recast.cast.author.display_name || recast.cast.author.username || '',
...
async function syncHistoricalBuilds() {
    try {
        ...
        console.log('- Oldest build:', oldestBuild ? new Date(oldestBuild.publishedAt).toISOString() : 'none');
        console.log('- Newest build:', newestBuild ? new Date(newestBuild.publishedAt).toISOString() : 'none');
        ...
        while (consecutiveEmptyResponses < MAX_EMPTY_RESPONSES) {
            try {
                ...
                if (!casts || casts.length === 0) {
                    ...
                    console.log(`No casts found. Empty response count: ${consecutiveEmptyResponses}/${MAX_EMPTY_RESPONSES}`);
                ...
                console.log(`- Claims: ${stats.replies} replies, ${stats.quotes} quotes (${stats.tagged} tagged)`);
                console.log(`- Batch processing time: ${batchDuration}ms`);
                console.log(`- Average time per batch: ${avgTimePerBatch.toFixed(2)}ms`);

                console.log('\nOverall Progress:');
                console.log(`- Total builds processed: ${totalProcessed}`);
                console.log(`- Total claims: ${totalReplies + totalQuotes} (${totalTaggedClaims} tagged)`);
                console.log(`- Total unique users: ${totalUsers.size}`);
                console.log(`- Total time elapsed: ${((Date.now() - startTime) / 1000).toFixed(2)}s`);
        ...
        console.log('- Oldest build:', finalOldest ? new Date(finalOldest.publishedAt).toISOString() : 'none');
        console.log('- Newest build:', finalNewest ? new Date(finalNewest.publishedAt).toISOString() : 'none');
        console.log('\nSync Statistics:');
        console.log('- Total builds processed:', totalProcessed);
