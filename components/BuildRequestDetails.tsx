...
export function BuildRequestDetails({ buildRequest, claims }: BuildRequestDetailsProps) {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-purple-900 via-purple-800 to-purple-700 relative">
            ...
            <div className="container mx-auto px-4 py-8">
                <Link href="/" className="text-purple-200 hover:text-white mb-4 inline-flex items-center transition-colors">
                ...
                <div className="bg-purple-800/30 rounded-xl p-6 mb-8 shadow-xl backdrop-blur-sm border border-purple-700/50">
                    <div className="flex items-center mb-4">
                        ...
                        <div>
                            <h2 className="font-semibold text-purple-100">{buildRequest.author.display_name || buildRequest.author.username}</h2>
                            <p className="text-purple-300">@{buildRequest.author.username}</p>
                        </div>
                    </div>

                    <p className="text-lg mb-4 text-purple-100 whitespace-pre-wrap">{buildRequest.text}</p>
                            ...
                            {buildRequest.embeds.map((embed, index) => (
                                <div key={index}>
                                    {embed.cast ? (
                                        ...
                                        >
                                            {embed.metadata?.html?.ogTitle && (
                                                <h3 className="font-semibold text-purple-100">{embed.metadata.html.ogTitle}</h3>
                                            )}
                                            {embed.metadata?.html?.ogDescription && (
                                                <p className="text-purple-300 mt-1">{embed.metadata.html.ogDescription}</p>
                                            )}
                                        </a>
                                    ) : null}
                                </div>
                    ...
                    <button
                        onClick={() => isAuthenticated ? setIsClaimModalOpen(true) : setIsAuthModalOpen(true)}
                ...
                <div className="space-y-4">
                    <h2 className="text-2xl font-bold mb-4 text-purple-100">Claims ({claims.length})</h2>
                    {claims.length === 0 ? (
                        <div className="bg-purple-800/30 rounded-xl p-8 shadow-xl backdrop-blur-sm border border-purple-700/50 text-center">
                            ...
                            <h3 className="text-lg font-medium text-purple-100 mb-2">No Claims Yet</h3>
                            <p className="text-purple-300">
                                Looks like no one has built this yet. This could be your chance to shine! Why not be the first to build it?
                            </p>
                            <button
                                onClick={() => isAuthenticated ? setIsClaimModalOpen(true) : setIsAuthModalOpen(true)}
                        ...
                        claims.map((claim) => (
                            <div key={claim.castHash} className="bg-purple-800/30 rounded-xl p-6 shadow-xl backdrop-blur-sm border border-purple-700/50">
                                <div className="flex items-center mb-4">
                                    {claim.author.pfpUrl && (
                                        <SafeImage
                                    ...
                                    <div>
                                        <h3 className="font-semibold text-purple-100">{claim.author.displayName}</h3>
                                        <p className="text-purple-300">@{claim.author.username}</p>
                                    </div>
                                </div>
                                <p className="text-purple-100 whitespace-pre-wrap">{claim.text}</p>
                            </div>
                        ))
                    )}
                </div>
